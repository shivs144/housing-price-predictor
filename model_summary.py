...
# Load the data and preprocess (same as in save_models.py)
...
try:
    ...
    print(f"   Silhouette Score: {silhouette_score(cluster_data, cluster_labels):.4f}")
    ...
    for cluster_id in range(3):
        ...
        print(f"\n   Cluster {cluster_id}:")
        print(f"     Size: {len(cluster_data)} properties")
        print(f"     Mean Residual: {cluster_data['Residual'].mean():.2f}")
        print(f"     Mean Predicted Price: ₹{cluster_data['Predicted_Price'].mean():,.0f}")
        print(f"     Mean Actual Price: ₹{cluster_data['Actual_Price'].mean():,.0f}")
        print(f"     Residual Std Dev: {cluster_data['Residual'].std():.2f}")
    ...
    for cluster_id, label in label_map.items():
        cluster_data = cluster_df[cluster_df['Cluster'] == cluster_id]
        print(f"   {label}: {len(cluster_data)} properties")
...
print(f"   K-means Silhouette Score: {silhouette_score(cluster_data, cluster_labels):.4f}")
